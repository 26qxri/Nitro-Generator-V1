    ...
    def main(self):  # The main function contains the most important code
        ...
        if os.name == "nt":  # If the system is windows
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Nitro Generator Made by EzzHacks")  # Change the
        else:  # Or if it is unix
            print(f'\33]0;Nitro Generator and Checker - Made by EzzHacks (26.qxri)\a',
                  end='', flush=True)  # Update title of command prompt

        print("""Nitro Generator by EzzHacks (26.qxri#2022)
Discord Nitro Generator Beta V1
Discord: discord.gg/ezzhacks

""")  # Print the title card
        time.sleep(2)  # Wait a few seconds
        # Print who developed the code
        self.slowType("Made by: 26.qxri#2022", .02)
        time.sleep(1)  # Wait a little more
        # Print the first question
        self.slowType(
            "\nWie viele Codes müssen generiert und überprüft werden: ", .02, newLine=False)
        ...
        for s in c:  # Loop over the amount of codes to check
            try:
                ...
                print(f" Invalid | {url} ")  # Tell the user an error occurred

            if os.name == "nt":  # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by EzzHacks (26.qxri)")  # Change the title
                print("")
            else:  # If it is a unix system
                # Change the title
                print(
                    f'\33]0;Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by EzzHacks (26.qxri)\a', end='', flush=True)
...
Results:
 ...
 Valid Codes: {', '.join(valid)}""")  # Give a report of the results of the check
        ...
        input("\nDas Ende! Drücken Sie 5x die Eingabetaste, um das Programm zu schließen.")
